...
class MultiAgentWrapper(Wrapper):
    ...
    """
    A Wrapper for Multi-Agent environments which contain multiple agents.
    """

    def __init__(self, env):
        super(MultiAgentWrapper, self).__init__(env)
        assert env.n_agents > 1, "Multi Agent Wrapper for Multi-Agent Environments requires more than one agent."
        self.action_space = env.action_space

    def reset(self):
        return super(MultiAgentWrapper, self).reset()

    def step(self, actions):
        return super(MultiAgentWrapper, self).step(actions)
